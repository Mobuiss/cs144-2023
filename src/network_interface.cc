...
void NetworkInterface::send_datagram( const InternetDatagram& dgram, const Address& next_hop )
{
  ...
  if ( arp_list.find( next_ip ) != arp_list.end() && time_now - arp_list[next_ip].second <= update_time ) {
    auto data = serialize( dgram );
    cout << "second=" << arp_list[next_ip].second << "|" << to_string( arp_list[next_ip].first ) << endl;
...
optional<InternetDatagram> NetworkInterface::recv_frame( const EthernetFrame& frame )
{
  ...
  if ( frame.header.type == EthernetHeader::TYPE_IPv4 ) {
    ...
    if ( msg.opcode == ARPMessage::OPCODE_REQUEST ) {
      ...
      if ( wait_for_send.find( msg.sender_ip_address ) == wait_for_send.end() )
        return nullopt;
      EthernetFrame ef;
      ef.header.src = ethernet_address_;
