  ...
  for(auto &aif:interfaces_){
    ...
    while((op_dgram=aif.maybe_receive()).has_value()){
      ...
      for(size_t i=0;i<table.size();++i){
        if(table[i].prefix_length>len&&(table[i].prefix_length==0||(dgram.header.dst>>(
          32-table[i].prefix_length))==(table[i].route_prefix>>(32-table[i].prefix_length)))){
          pos=i;
          len=table[i].prefix_length;
        }
      }
      if(len==-1)continue;
      interface(table[pos].interface_num).send_datagram(
        dgram,table[pos].next_hop.value_or(Address::from_ipv4_numeric(dgram.header.dst)));
    }
  }
}
